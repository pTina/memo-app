...
- **폰트 커스터마이징** — `.ttf` / `.otf` / `.woff` / `.woff2` 폰트 파일을 업로드해 등록하고, 메모마다 서로 다른 폰트를 지정
- **선택 텍스트 스타일링** — 글자를 드래그하면 뜨는 플로팅 툴바로 글자색, 형광펜(4색), 자간, 행간을 선택 영역에만 적용
...
- **다크/라이트 모드** — 시스템 설정을 기본값으로 따르며 수동 전환 가능, 선택한 값은 재방문 시에도 유지
