...
백업 모달에 파일 이름 입력란이 추가됨. 비워두면 기존처럼 `memo-backup-타임스탬프.json`으로 저장되고, 입력하면 그 이름으로 저장됨. 파일명에 쓸 수 없는 문자(`\ / : * ? " < > |`)는 자동으로 제거됨.

**백업 다운로드 전 확인 모달**

"백업 파일 다운로드" 버튼을 눌러도 바로 받아지지 않고, 몇 건이 내려받아지는지(선택한 메모 개수, 전체 메모 중 몇 건인지, 선택한 것이 전체와 같으면 "전체 선택됨" 표시)와 저장될 파일명을 보여주는 확인 모달이 먼저 뜸. "이름 바꾸기"를 누르면 모달만 닫히고 파일 이름 입력란으로 포커스가 이동해 바로 수정할 수 있고, "확인"을 눌러야 실제 다운로드가 시작됨.
