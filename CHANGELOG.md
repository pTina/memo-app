...
| AS-IS | TO-BE |
|---|---|
| ![](img/changelog/slash-time-before.png) | ![](img/changelog/slash-time-after.png) |

날짜만 삽입되던 슬래시 커맨드에 시간(시:분)이 추가됨. 시간은 직접 입력할 수 없고 항상 현재 시각이 자동으로 채워지며, 잠금 아이콘이 붙은 시간 칩을 클릭(또는 키보드로 포커스 후 Space/Enter)하면 포함(🔒)/제외(🔓)를 켜고 끌 수 있음. 기본값은 켜짐이며, 설정 팝업에 추가된 "날짜 삽입 시 시간 포함" 토글로 기본값을 바꿀 수 있고 백업/복원에도 반영됨. 날짜 형식은 `2026/08/04(화) 14:38`.

### 변경

- 슬래시 날짜 팝오버 레이아웃 정리: "삽입 결과" 미리보기 영역 제거, 삽입 버튼을 시간 칩 옆으로 옮기고 고스트 스타일(투명 배경 + 테두리)로 변경, 모바일에서는 삽입 버튼만 하단 풀와이드로 표시
